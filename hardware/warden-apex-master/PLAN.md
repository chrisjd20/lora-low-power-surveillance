...
Variant sanity-check (all three zips verified against `tools/variants.yaml`
and the live PCB footprint set):

| Tier | Board footprints | DNP | Expected in BOM/POS | BOM ct | POS ct |
|---|---:|---:|---:|---:|---:|
| drone | 94 | 24 | 61 | 66 | 61 |
| cell_master | 94 | 13 | 72 | 77 | 70 (JP1/JP2 are silk jumpers) |
| apex | 94 | 3 | 82 | 87 | 78 (JP1..JP4 are silk jumpers) |

No DNP leakage into BOM/POS. All required footprints present. Gerbers
are byte-identical across tiers; drill files are geometrically
identical (the only diff is the `date` comment line).

Use this top snapshot as the authoritative current state; earlier phase
sections below are retained as historical execution record.
...
## Phase 23 — SIM7080/UART/PWRKEY blocker-closure attempt (in progress)

This phase attempted to close the six live blockers called out in the
handoff (missing SIM7080 pads on PCB instance, UART level mismatch,
missing PWRKEY control, unwired U5 channels, low modem/sat bulk caps,
and poor C15 REGN bypass locality) using deterministic scripting
(`pcbnew` API + direct file patching) instead of MCP
`sync_schematic_to_board`.

### What was changed in this pass

- Board-side `IC1` footprint instance was reloaded from
  `LCC-42_SIM7080G.kicad_mod` so the full-pad package is present on the
  live PCB instance (including pad numbers previously missing on the old
  reduced instance).
- New UART translation chain was added (`Q4`, `Q5`, `R25`..`R28`) and
  nets were split into mixed-voltage rails (`UART1_TX_1V8`,
  `UART1_RX_1V8`) with corresponding schematic labels and board net
  assignments.
- `MODEM_PWRKEY_N` control net was introduced and tied between `IC1.39`
  and `U1.21`.
- U5 protection channels were assigned to SIM rails (`SIM_VDD`,
  `SIM_DATA`, `SIM_CLK`, `SIM_RST`) at both schematic-net and PCB-pad
  level.
- `C31` and `C34` values were increased to `220uF` for additional modem
  and satellite burst support.
- `C15` was moved close to the charger region as part of REGN locality
  tightening.
- Variant artifacts were regenerated by rerunning:
  `python3 tools/phase12_variants.py`.

### Current validation snapshot

Latest verification run on this in-progress state:
...
So this phase is **not yet in a release-ready/fab-ready state**. The
remaining error-level issues are concentrated around:

- temporary translator placement density (`R25`..`R28`) and courtyard/
  pad spacing,
- residual `/DUMMY_NET_*` via interactions against GND pours from the
  current autoroute workflow,
- edge-clearance and a few starved-thermal/unconnected items after the
  latest reroute pass.

### Status

This section records the state accurately for handoff/recovery. Further
cleanup is required before claiming restored `--severity-error` DRC
cleanliness.

## Phase 24 — Board envelope expansion (+asymmetric margins)

Per the mechanical-change request, the board outline was expanded while
keeping functional circuitry placements unchanged:
...
This changes the edge-cuts rectangle from `100 x 100 mm` to
`125 x 125 mm` with new extents:

- X: `-15 mm .. 110 mm`
- Y: `-5 mm .. 120 mm`

### Mounting-hole relocation

`MH1..MH4` were moved to the new corners with the same 3.5 mm edge inset
as before:
...
This outline change plus the subsequent full-zone boundary extension
improved edge-clearance pressure in the previous state (error-level DRC
dropped from 28 to 22, then to 10 after zone refill), but the board
remains an in-progress engineering state and is not yet fab-signoff
clean.

### Regenerated outputs

`python3 tools/phase12_variants.py` was re-run after the outline/hole
change, regenerating:
